...
function LeadRequest({ multiple = true, onSelect }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setPopup("approve");
      setTimeout(() => {
        navigate("/lead/lead-permission-history");
      }, 2000);
    } catch (err) {
    ...
    <section id="staffPage">
      ...
      <div className="allowInfo">
        ...
        <form>
          ...
          <div className="box4 hasSubmenu" style={{ cursor: "pointer" }} onClick={() => toggle("deadline")}>
            <label>Son tarix</label>
            <div className={`icon ${open.deadline ? "" : "active"}`}>
              <MdOutlineKeyboardArrowDown />
            </div>
            <p>
              {deadlineDate ? deadlineDate : "Başlama və bitmə tarixini seçin"}
            </p>
            <div
