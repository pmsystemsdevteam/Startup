...
function LeadRequestPage({ multiple = true, onSelect }) {
  ...
  useEffect(() => {
    ...
    if (userId) {
      fetchUserData(userId).then((data) => {
        if (data?.company?.id) {
          fetchTeamMembers(data.company.id);
        }
      });
    }
  }, [navigate]);
  ...
  const handleSubmit = async () => {
    ...
    if (selectedAssigned.length === 0) {
      errors.push("• Tapşırıq üçün işçi seçilməyib.");
    }
  ...
  return (
    <section id="staffPage">
      ...
      <div className="confirmBox">
        <h2>Tapşırıq verilən şəxslər</h2>
        <div className="chosenPerson" onClick={() => toggle("deadline")}>
          <label>Təyin et</label>
          <div className="icon">
            <MdOutlineKeyboardArrowDown />
          </div>
          <p>
...
export default LeadRequestPage;
