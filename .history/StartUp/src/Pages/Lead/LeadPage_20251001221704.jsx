...
function LeadPage() {
  ...
  // Handle status select with loading
  const handleStatusSelect = async (row, newStatus) => {
  ...
  // Backend approve/reject request
  const submitApproval = async (formId, decision, reason = "") => {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/hr/forms/${formId}/approve/`,
        {
      ...
      alert(
        decision === "approved" ? "Form təsdiq edildi!" : "Form rədd edildi!"
      );
    } catch (error) {
  ...
  // Submit rejection with loading
  const submitRejection = async () => {
    if (!selectedFormId) return;
    setLoading(true);
    await submitApproval(selectedFormId, "rejected", rejectReason.trim());
    setPopup(null);
    setRejectReason("");
    ...
    <div className="leadPage">
      ...
      >
        <div className="normalPop" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="button" onClick={submitRejection} disabled={loading}>
            Göndər
          </div>
        </div>
      </div>
                ...
                <td>
                  <div className="statusBox">
                    <span
                      ...
                      onClick={() => !loading && toggleStatusDropdown(absIndex)}
                    ...
                    {openStatusDropdown === absIndex && (
                      <div
                        className="statusDropdown"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {getOtherStatuses(row.status).map((status, idx) => (
                          <span
                            key={idx}
                            className={getStatusClass(status)}
                            onClick={() => handleStatusSelect(row, status)}
                            style={{
